...
export async function npmCall(line: string) {
  ...
  try {
    ...
    if (
      prs != null &&
      prs.data != null &&
      prs.data.repository != null &&
      prs.data.repository.type != null &&
      prs.data.repository.url != null
    ) {
      if (prs.data.repository.type == 'git') {
        let requestedURL: string = prs.data.repository.url;
        if (requestedURL.includes(githubURLMatch)) {
          var result = requestedURL.substring(
            requestedURL.lastIndexOf('github.com')
          );
          ...
          writeToFile(
            logFile,
            'Wrote npm now github link succesfully!: ' + result,
          ...
          writeToFile(
            logFile,
            'No git repository found for this package. At this time this tool only supports npm modules hosted on github repositories',
            'error in writing to log file'
          );
...
export function lineMatching(line: string) {
  if (line.includes(npmURLMatch)) {
    ...
    writeToFile(
      logFile,
      'neither url link identiied, check input format. provided url: ' + line,
      'error in writing to log file'
    );
    return 'noMatch';
  }
