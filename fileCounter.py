...
def calcCorrectness(clocOut):
    ...
    correctness = clocOut[3] / diff * 0.6 + clocOut[3] * 0.001
...
def createClocFile(repoDir, outputFile):
    ...
    cmd = (
        clocLoc
        + " --csv --include-lang=Markdown,Text,TeX "
        + repoDir
        + " | tail -n 1 >> "
        + outputFile
    )
...
def createJSONFile(tmpInfo):
    ...
    jsonString += (
        ', "CORRECTNESS_SCORE":' + tmpInfo[4] + ', "BUS_FACTOR_SCORE":' + tmpInfo[7]
    )
    jsonString += (
    ...
    dictionary = {
        ...
        "Correctness": float(tmpInfo[4]),
        "RampUp": float(tmpInfo[3]),
        "ResponsiveMaintainer": float(tmpInfo[6]),
        "LicenseScore": float(tmpInfo[5]),
        "GoodPinningPractice": 0,
        "PullRequest": 0,
        "NET_SCORE": float(tmpInfo[8]),
    }
    jsonFile = json.dumps(dictionary, indent=2)
    print(jsonFile)
    return jsonString
